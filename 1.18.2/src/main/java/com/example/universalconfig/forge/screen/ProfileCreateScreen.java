...
public final class ProfileCreateScreen extends Screen {
    ...
    public ProfileCreateScreen(Screen parent) {
        super(new TranslatableComponent("screen.universal_config.profile_create_title"));
        this.parent = parent;
    }
    ...
    protected void init() {
        ...
        descriptionField = new EditBox(font, formLeft, 92, FORM_WIDTH, 20,
                new TranslatableComponent("screen.universal_config.profile_description_placeholder"));
        ...
        keybindsCheckbox = new Checkbox(formLeft, CHECKBOX_Y, FORM_WIDTH, 20,
                new TranslatableComponent("screen.universal_config.target_keybinds"), includeKeybinds, true);
        clientOptionsCheckbox = new Checkbox(formLeft, CHECKBOX_Y + CHECKBOX_STEP, FORM_WIDTH, 20,
                new TranslatableComponent("screen.universal_config.target_client"), includeClientOptions, true);
        modConfigsCheckbox = new Checkbox(formLeft, CHECKBOX_Y + CHECKBOX_STEP * 2, FORM_WIDTH, 20,
                new TranslatableComponent("screen.universal_config.target_mods"), includeModConfigs, true);
        addRenderableWidget(keybindsCheckbox);
        addRenderableWidget(clientOptionsCheckbox);
        addRenderableWidget(modConfigsCheckbox);
        addRenderableWidget(Button.builder(new TranslatableComponent("screen.universal_config.save"), button -> create())
    ...
    public void render(PoseStack context, int mouseX, int mouseY, float delta) {
        ...
        drawString(context, font, new TranslatableComponent("screen.universal_config.profile_name_label"),
                formLeft, 40, 0xDDDDDD);
        drawString(context, font, new TranslatableComponent("screen.universal_config.profile_description_label"),
                formLeft, 80, 0xDDDDDD);
        drawString(context, font, new TranslatableComponent("screen.universal_config.profile_save_contents"),
        ...
        ScreenUtil.drawProfileIcon(formLeft + 2, NAME_FIELD_Y + 2, 16, selectedIconId);
    }
