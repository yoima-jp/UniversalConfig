...
public final class ProfileConfirmScreen extends Screen {
    ...
    public ProfileConfirmScreen(Screen parent, Path profilePath) {
        super(new TranslatableComponent("screen.universal_config.profile_confirm_title"));
        this.parent = parent;
    ...
    private void loadDiff() {
        try {
            ...
            status = ex instanceof UniversalConfigException universalConfigException
                    ...
                    : new TranslatableComponent("screen.universal_config.confirm_load_failed");
    ...
    private void rebuildButtons() {
        ...
        if (manifest != null && !detailLines.isEmpty()) {
            Component label = new TranslatableComponent(detailsVisible
    ...
    private List<DisplayLine> buildDetailLines() {
        ...
        result.add(new DisplayLine(new TranslatableComponent("screen.universal_config.confirm_source",
        ...
        result.add(new DisplayLine(new TranslatableComponent("screen.universal_config.confirm_current",
                ScreenUtil.environment().minecraftVersion(), ScreenUtil.environment().loaderId(),
                ScreenUtil.environment().loaderVersion()), false));
        result.add(new DisplayLine(new TranslatableComponent("screen.universal_config.confirm_compatibility",
        ...
        result.add(new DisplayLine(new TranslatableComponent("screen.universal_config.confirm_tested",
                testedVersions()), false));
        result.add(new DisplayLine(new TranslatableComponent("screen.universal_config.confirm_apply_mode"), false));
        result.add(new DisplayLine(new TranslatableComponent("screen.universal_config.confirm_risk",
                new TranslatableComponent(riskLevelKey(diff.riskLevel))), diff.riskLevel == RiskLevel.HIGH));
    ...
    private void appendSection(List<DisplayLine> result, String titleKey, List<String> values, boolean warning) {
        ...
        result.add(new DisplayLine(new TranslatableComponent(titleKey), warning));
        for (String value : values) {
            result.add(new DisplayLine(new TranslatableComponent("screen.universal_config.confirm_list_item", value), warning));
        }
    }

    private Component scheduleButtonLabel() {
        return switch (diff.riskLevel) {
            case HIGH -> new TranslatableComponent("screen.universal_config.confirm_schedule_high");
            case MEDIUM -> new TranslatableComponent("screen.universal_config.use_profile");
            case LOW -> new TranslatableComponent("screen.universal_config.use_profile");
        };
    }
    ...
    private void schedule() {
        if (manifest == null) {
            status = new TranslatableComponent("screen.universal_config.confirm_load_failed");
            return;
        }
        try {
            ...
            status = ex instanceof UniversalConfigException universalConfigException
                    ? ScreenUtil.errorText(universalConfigException)
                    : new TranslatableComponent("screen.universal_config.schedule_failed");
    ...
    private void drawPanel(PoseStack context) {
        ...
        int y = panelTop() + 16;
        drawString(context, font, new TextComponent(safe(manifest.name)), x, y, 0xFFFFFFFF);
        y += 18;
        ...
        drawTrimmed(context, new TranslatableComponent("screen.universal_config.confirm_environment", minecraftVersion, loader).getString(),
        ...
        drawString(context, font, new TranslatableComponent("screen.universal_config.included_settings"), x, y, 0xFFFFFFFF);
        y += 16;
    ...
    private void drawDetails(PoseStack context, int x, int detailsTop, int contentRight) {
    ...
    private int drawInclude(PoseStack context, boolean included, String key, int x, int y) {
        if (included) {
            ...
            drawString(context, font, new TranslatableComponent(key), x + 14, y, 0xFFFFFFFF);
    ...
    private void drawBorderedRect(PoseStack context, int left, int top, int right, int bottom, int fill) {
