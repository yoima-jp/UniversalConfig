...
public final class ApplyScheduledScreen extends Screen {
    ...
    private ApplyScheduledScreen(Screen parent, State state) {
        super(new TranslatableComponent(titleKey(state)));
        this.parent = parent;
        this.state = state;
    }
    ...
    private void initScheduledButtons() {
        ...
        Component laterLabel = new TranslatableComponent("screen.universal_config.apply_scheduled_later");
    ...
    private void initErrorButtons() {
        int panelTop = panelTop();
        Component backLabel = new TranslatableComponent("screen.universal_config.back");
        ...
        Component quitLabel = new TranslatableComponent("screen.universal_config.quit");
    ...
    public void render(PoseStack context, int mouseX, int mouseY, float delta) {
        ...
        drawCenteredString(context, font, new TranslatableComponent(firstLineKey), width / 2, textY, MUTED_TEXT_COLOR);
        drawCenteredString(context, font, new TranslatableComponent(secondLineKey), width / 2, textY + 18, MUTED_TEXT_COLOR);
        if (state == State.RESTART_FAILED) {
            ...
            drawWrappedCenteredText(context, new TranslatableComponent("screen.universal_config.restart_failed_line3"),
                    textY + 36, MUTED_TEXT_COLOR);
        } else if (cancelError) {
            drawWrappedCenteredText(context, new TranslatableComponent("screen.universal_config.cancel_failed"),
                    textY + 36, 0xFFFF7777);
        }
    ...
    private void drawWrappedCenteredText(PoseStack context, Component message, int startY, int color) {
        ...
        for (FormattedCharSequence line : lines) {
            ...
            drawString(context, font, line, lineX, lineY, color);
            lineY += 10;
        }
    }
    ...
    private int scheduledButtonWidth() {
        int action = pairButtonWidth(
                new TranslatableComponent("screen.universal_config.apply_scheduled_later"),
                new TranslatableComponent("screen.universal_config.restart_now"));
        int cancel = measureButton(new TranslatableComponent("screen.universal_config.apply_scheduled_cancel"));
        return Math.max(action, cancel);
    }

    private int errorButtonWidth() {
        return pairButtonWidth(
                new TranslatableComponent("screen.universal_config.back"),
                new TranslatableComponent("screen.universal_config.quit"));
    }
