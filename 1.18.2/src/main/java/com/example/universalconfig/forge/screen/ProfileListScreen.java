...
public final class ProfileListScreen extends Screen {
    ...
    public ProfileListScreen(Screen parent) {
        super(new TranslatableComponent("screen.universal_config.title"));
        this.parent = parent;
    }
    ...
    private void reload() {
        ...
        try {
            ...
            status = ex instanceof UniversalConfigException
                    ...
                    : new TranslatableComponent("screen.universal_config.load_failed");
        }
    }
    ...
    private void rebuildButtons() {
        ...
        for (int index = firstRow; index < lastRow; index++) {
            ...
            MutableComponent narration = new TextComponent(profileName(profile.manifest()));
        ...
        if (selected != null) {
            ...
            addRenderableWidget(Button.builder(new TranslatableComponent("screen.universal_config.use_profile"), button -> openConfirm(path))
                    .bounds(detailX(), primaryButtonY(), actionWidth, PRIMARY_BUTTON_HEIGHT).build());
            addRenderableWidget(Button.builder(new TranslatableComponent("screen.universal_config.more"), button -> {
        ...
        if (pendingImport != null) {
            Component restartLabel = new TranslatableComponent("screen.universal_config.restart_now");
            Component cancelLabel = new TranslatableComponent("screen.universal_config.apply_scheduled_cancel");
        ...
        MutableComponent closeNarration = new TranslatableComponent("screen.universal_config.close");
        addRenderableWidget(Button.builder(new TextComponent("×"), button -> onClose())
    ...
    private void addMoreMenuButtons(Path path) {
        ...
        Component defaultLabel = new TranslatableComponent(isDefaultProfile(path)
        ...
        addMoreMenuButton(Button.builder(new TranslatableComponent("screen.universal_config.open_folder"), button -> openProfileDirectory())
                .bounds(x, y + BUTTON_HEIGHT + MENU_ITEM_GAP, width, BUTTON_HEIGHT).build());
        addMoreMenuButton(Button.builder(new TranslatableComponent("screen.universal_config.rename"), button -> rename(path))
                .bounds(x, y + (BUTTON_HEIGHT + MENU_ITEM_GAP) * 2, width, BUTTON_HEIGHT).build());
        addMoreMenuButton(Button.builder(new TranslatableComponent("screen.universal_config.duplicate"), button -> duplicate(path))
                .bounds(x, y + (BUTTON_HEIGHT + MENU_ITEM_GAP) * 3, width, BUTTON_HEIGHT).build());
        addMoreMenuButton(Button.builder(new TranslatableComponent("screen.universal_config.backups"), button -> openBackups())
                .bounds(x, y + (BUTTON_HEIGHT + MENU_ITEM_GAP) * 4, width, BUTTON_HEIGHT).build());
        addMoreMenuButton(Button.builder(new TranslatableComponent("screen.universal_config.delete"), button -> confirmDelete(path))
    ...
    private void addReorderButtons(int profileIndex, int cardY) {
        ...
        int y = cardY + 3;
        Button upButton = Button.builder(new TextComponent("\u2191"), button -> moveProfile(profileIndex, profileIndex - 1))
                .createNarration(ignored -> new TranslatableComponent("screen.universal_config.move_up"))
                .bounds(x, y, REORDER_BUTTON_WIDTH, REORDER_BUTTON_HEIGHT).build();
        upButton.active = profileIndex > 0;
        Button downButton = Button.builder(new TextComponent("\u2193"), button -> moveProfile(profileIndex, profileIndex + 1))
                .createNarration(ignored -> new TranslatableComponent("screen.universal_config.move_down"))
    ...
    private void openConfirm(Path path) {
        try {
            ...
            status = new TranslatableComponent("screen.universal_config.open_confirm_failed");
        }
    }
    ...
    private void confirmImport(Path source) {
        try {
            ...
            }, new TranslatableComponent("screen.universal_config.import_confirm", name),
                    new TranslatableComponent("screen.universal_config.import_warning")));
        } catch (UniversalConfigException | RuntimeException ex) {
            FileOperationLogger.failure("OPEN_IMPORT_CONFIRM", source, "invalid dropped profile", ex);
            status = new TranslatableComponent("screen.universal_config.import_failed");
            rebuildButtons();
        }
    }
    ...
    private void confirmDelete(Path path) {
        ...
        try {
            ...
            }, new TranslatableComponent("screen.universal_config.delete_confirm", name),
                    new TranslatableComponent("screen.universal_config.delete_warning")));
        } catch (RuntimeException ex) {
            FileOperationLogger.failure("OPEN_DELETE_CONFIRM", path, "failed", ex);
            status = new TranslatableComponent("screen.universal_config.delete_failed");
        }
    }
    ...
    public void onFilesDrop(List<Path> paths) {
        if (paths == null || paths.size() != 1 || !isDroppedProfileFile(paths.get(0))) {
            status = new TranslatableComponent("screen.universal_config.drop_one_profile");
            rebuildButtons();
            return;
        }
    ...
    private void cancelPendingApply() {
        ...
        try {
            ...
            status = new TranslatableComponent("screen.universal_config.cancel_failed");
        } catch (RuntimeException ex) {
    ...
    private void handleActionFailure(Exception ex, String fallbackKey) {
        if (ex instanceof UniversalConfigException) {
            ...
            status = new TranslatableComponent(fallbackKey);
        }
    }
    ...
    private void drawHeader(PoseStack context) {
        ...
        drawCenteredString(context, font, new TranslatableComponent("screen.universal_config.subtitle"),
        ...
        if (pendingImport != null) {
            ...
            int actionWidth = pendingButtonWidth(new TranslatableComponent("screen.universal_config.restart_now"))
                    + pendingButtonWidth(new TranslatableComponent("screen.universal_config.apply_scheduled_cancel")) + 6;
            int availableWidth = contentWidth() - actionWidth - 34;
            drawTrimmed(context, new TranslatableComponent("screen.universal_config.pending_named", pendingProfileName()).getString(),
    ...
    private void drawListContent(PoseStack context) {
        drawPanelHeader(context, contentLeft(), leftPanelRight(), new TranslatableComponent("screen.universal_config.saved_settings"));
        if (profiles.isEmpty()) {
    ...
    private void drawDetailContent(PoseStack context) {
        ...
        Component heading = selected == null
                ? new TranslatableComponent("screen.universal_config.profile_detail")
                : new TextComponent(profileName(selected.manifest()));
        ...
        drawString(context, font, new TranslatableComponent("screen.universal_config.included_settings"),
    ...
    private List<FormattedCharSequence> descriptionLines(ProfileManifest manifest) {
        return font.split(new TextComponent(descriptionSummary(manifest)), detailContentWidth());
    }
    ...
    private String environmentSummary(ProfileManifest manifest) {
        ...
        return new TranslatableComponent("screen.universal_config.confirm_environment", minecraftVersion, loader).getString();
    }
    ...
    private String loaderSummary(ProfileManifest manifest) {
        ...
        return new TranslatableComponent("screen.universal_config.loader_summary", loader, loaderVersion).getString();
    }
    ...
    private int drawInclude(PoseStack context, boolean included, String key, int x, int y) {
        if (included) {
            drawString(context, font, new TextComponent("✓"), x, y, CHECK_COLOR);
            drawTrimmed(context, new TranslatableComponent(key).getString(), x + 14, y, detailWidth() - 34, 0xFFFFFFFF);
            return y + 15;
        }
        return y;
    }
    ...
    private void drawProfileIcon(PoseStack context, int x, int y, int size, String iconId) {
        ...
        // ItemRendererは画面のPoseStackではなくRenderSystemのModelViewStackを参照する。
        // 画面側だけを変形すると(0,0)へ描かれてscissorに消えるため、実描画スタックを更新する。
    ...
    private String[] environmentLines(ProfileManifest manifest) {
        ...
        return new String[]{new TranslatableComponent("screen.universal_config.minecraft_version",
