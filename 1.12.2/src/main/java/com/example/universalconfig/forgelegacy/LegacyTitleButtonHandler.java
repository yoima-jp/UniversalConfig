...
public final class LegacyTitleButtonHandler {
    ...
    // 1.12.2のGuiMainMenuは height-10 の位置に文字列を描画するため、その上に最小限の間隔を確保する。
    ...
    public void onInitGui(GuiScreenEvent.InitGuiEvent.Post event) {
        if (event.getGui() instanceof GuiMainMenu) {
            ...
            event.getButtonList().removeIf(button -> button instanceof TitleIconButton);
            int y = titleButtonY(event.getGui().height);
            event.getButtonList().add(new TitleIconButton(BUTTON_ID, BUTTON_MARGIN, y));
        }
    }
