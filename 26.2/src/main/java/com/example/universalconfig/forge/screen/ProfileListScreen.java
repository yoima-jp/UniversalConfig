...
public final class ProfileListScreen extends Screen {
    ...
    // 26.1.2のGUIアイテムアトラスは16px固定だが、ScreenUtil.drawProfileIconはPiP経路で
    // ターゲット解像度(size*guiScale)のテクスチャへ直接描画するため、28pxでも荒れない。
    ...
    private void openBackups() {
        minecraft.setScreenAndShow(new BackupListScreen(this));
    }
    ...
    private void moveProfile(int sourceIndex, int targetIndex) {
        if (sourceIndex < 0 || sourceIndex >= profiles.size() || targetIndex < 0
                || targetIndex >= profiles.size() || sourceIndex == targetIndex) return;
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double horizontalAmount, double verticalAmount) {
        if (mouseX >= contentLeft() && mouseX <= leftPanelRight()
            ...
            int delta = verticalAmount > 0 ? -LIST_SCROLL_STEP : verticalAmount < 0 ? LIST_SCROLL_STEP : 0;
        ...
        if (mouseX >= rightPanelLeft() && mouseX <= rightPanelRight()
            ...
            int delta = verticalAmount > 0 ? -DETAIL_SCROLL_STEP : verticalAmount < 0 ? DETAIL_SCROLL_STEP : 0;
            detailScroll = Math.max(0, Math.min(maxDetailScroll(), detailScroll + delta));
            return detailScroll != previousScroll || verticalAmount != 0;
        }
    ...
    public boolean mouseDragged(MouseButtonEvent event, double deltaX, double deltaY) {
        if (event.button() == 0 && draggingProfileIndex >= 0) {
            if (!draggingProfile && Math.abs(event.y() - dragStartY) > 3) draggingProfile = true;
            if (draggingProfile) {
                dragTargetIndex = dropIndexAt(event.y());
                return true;
            }
        }
    ...
    public boolean mouseReleased(MouseButtonEvent event) {
        if (event.button() == 0 && draggingProfileIndex >= 0) {
            ...
            dragTargetIndex = -1;
            draggingProfile = false;
            if (wasDragging) moveProfile(sourceIndex, targetIndex);
            else if (profileIndexAt(event.x(), event.y()) == sourceIndex) selectProfile(sourceIndex);
            return true;
        }
    ...
    private int profileIndexAt(double mouseX, double mouseY) {
        ...
        return index >= 0 && index < profiles.size() && relativeY % CARD_STEP < CARD_HEIGHT ? index : -1;
    }
    ...
    private int reorderDeltaAt(double mouseX, double mouseY) {
        ...
        int x = cardX() + cardWidth() - REORDER_BUTTON_WIDTH - 4;
        int y = cardY(index) + 3;
        if (inside(mouseX, mouseY, x, y, REORDER_BUTTON_WIDTH, REORDER_BUTTON_HEIGHT)) return -1;
    ...
    private int dropIndexAt(double mouseY) {
        ...
        return Math.max(0, Math.min(profiles.size() - 1, relativeY / CARD_STEP));
    }
    ...
    private void drawHeader(GuiGraphicsExtractor context) {
        ...
        context.centeredText(font, Component.translatable("screen.universal_config.subtitle"),
    ...
    private void drawPanels(GuiGraphicsExtractor context) {
    ...
    private void drawListContent(GuiGraphicsExtractor context) {
    ...
    private void drawDetailContent(GuiGraphicsExtractor context) {
        ProfileSummary selected = selectedProfile();
        ...
        ScreenUtil.drawProfileIcon(context, x, y, PROFILE_ICON_SIZE, manifest.icon);
        ...
        for (FormattedCharSequence line : descriptionLines(manifest)) {
            context.text(font, line, x, y, SECONDARY_TEXT_COLOR);
        ...
        context.text(font, Component.translatable("screen.universal_config.included_settings"),
    ...
    private String profileFileName(ProfileSummary summary) {
        if (summary == null || summary.path() == null || summary.path().getFileName() == null) return "";
    ...
    private int drawInclude(GuiGraphicsExtractor context, boolean included, String key, int x, int y) {
        if (included) {
            context.text(font, Component.literal("✓"), x, y, CHECK_COLOR);
    ...
    private void drawPanelHeader(GuiGraphicsExtractor context, int left, int right, Component heading) {
        context.fill(left + 2, mainTop() + 2, right - 2, mainTop() + PANEL_HEADER_HEIGHT, PANEL_HEADER_COLOR);
        context.text(font, heading, left + PANEL_PADDING, mainTop() + 8, 0xFFFFFFFF);
        context.fill(left + 2, mainTop() + PANEL_HEADER_HEIGHT - 1, right - 2, mainTop() + PANEL_HEADER_HEIGHT, DIVIDER_COLOR);
    }

    private void drawProfileCards(GuiGraphicsExtractor context, int mouseX, int mouseY) {
        int firstRow = firstVisibleRow();
        int lastRow = lastVisibleRowExclusive();
        ...
        for (int index = firstRow; index < lastRow; index++) {
            ...
            ScreenUtil.drawProfileIcon(context, iconX, iconY, PROFILE_ICON_SIZE, manifest.icon);
    ...
    private void drawDragIndicator(GuiGraphicsExtractor context) {
        if (!draggingProfile || dragTargetIndex < 0 || dragTargetIndex == draggingProfileIndex) return;
        int y = dragTargetIndex > draggingProfileIndex
                ? cardY(dragTargetIndex) + CARD_HEIGHT - 1 : cardY(dragTargetIndex) - 1;
        context.fill(cardX(), y, cardX() + cardWidth(), y + 2, SELECTED_BORDER_COLOR);
    }

    private void drawReorderButtons(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        for (Button button : reorderButtons) button.extractRenderState(context, mouseX, mouseY, delta);
    }
    ...
    private void drawDetailScrollbar(GuiGraphicsExtractor context) {
    ...
    private void drawMoreMenuOverlay(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
    ...
    private void drawStatus(GuiGraphicsExtractor context) {
    ...
    private void drawBorderedRect(GuiGraphicsExtractor context, int left, int top, int right, int bottom,
    ...
    private void drawTrimmed(GuiGraphicsExtractor context, String text, int x, int y, int maxWidth, int color) {
        context.text(font,
    ...
    private static final class ProfileCardButton extends Button {
        private ProfileCardButton(int x, int y, int width, int height, MutableComponent narration, OnPress onPress) {
            super(x, y, width, height, Component.empty(), onPress, ignored -> narration);
        }
