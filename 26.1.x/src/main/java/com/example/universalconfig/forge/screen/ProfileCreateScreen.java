...
public final class ProfileCreateScreen extends Screen {
    ...
    protected void init() {
        ...
        keybindsCheckbox = Checkbox.builder(
                        Component.translatable("screen.universal_config.target_keybinds"), font)
                .pos(formLeft, CHECKBOX_Y).maxWidth(FORM_WIDTH).selected(includeKeybinds).build();
        clientOptionsCheckbox = Checkbox.builder(
                        Component.translatable("screen.universal_config.target_client"), font)
                .pos(formLeft, CHECKBOX_Y + CHECKBOX_STEP).maxWidth(FORM_WIDTH)
                .selected(includeClientOptions).build();
        modConfigsCheckbox = Checkbox.builder(
                        Component.translatable("screen.universal_config.target_mods"), font)
                .pos(formLeft, CHECKBOX_Y + CHECKBOX_STEP * 2).maxWidth(FORM_WIDTH)
                .selected(includeModConfigs).build();
