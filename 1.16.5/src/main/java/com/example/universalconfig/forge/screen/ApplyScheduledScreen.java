...
public final class ApplyScheduledScreen extends Screen {
    ...
    private ApplyScheduledScreen(Screen parent, State state) {
        super(new TranslationTextComponent(titleKey(state)));
        this.parent = parent;
        this.state = state;
    }
    ...
    private void initScheduledButtons() {
        ...
        ITextComponent laterLabel = new TranslationTextComponent("screen.universal_config.apply_scheduled_later");
        ...
        if (stack) {
            ...
            addButton(Button.builder(laterLabel, button -> onClose())
                    .bounds(width / 2 - buttonWidth / 2, firstActionY, buttonWidth, BUTTON_HEIGHT)
                    .build());
            addButton(Button.builder(restartLabel, button -> restartMinecraft())
    ...
    private void initErrorButtons() {
        int panelTop = panelTop();
        ITextComponent backLabel = new TranslationTextComponent("screen.universal_config.back");
        if (state == State.PENDING_INVALID) {
            ...
            addButton(Button.builder(backLabel, button -> onClose())
                    .bounds(width / 2 - buttonWidth / 2, buttonY, buttonWidth, BUTTON_HEIGHT)
                    .build());
            return;
        }

        ITextComponent quitLabel = new TranslationTextComponent("screen.universal_config.quit");
        ...
        if (stack) {
            ...
            addButton(Button.builder(backLabel, button -> onClose())
                    ...
                    .build());
            addButton(Button.builder(quitLabel, button -> quitMinecraft())
                    .bounds(width / 2 - buttonWidth / 2, bottomRowY, buttonWidth, BUTTON_HEIGHT)
                    .build());
        } else {
    ...
    public void render(MatrixStack context, int mouseX, int mouseY, float delta) {
        ...
        fill(context, panelLeft, panelTop, panelRight, panelBottom, PANEL_COLOR);
        fill(context, panelLeft, panelTop, panelRight, panelTop + 1, PANEL_BORDER_COLOR);
        fill(context, panelLeft, panelBottom - 1, panelRight, panelBottom, PANEL_BORDER_COLOR);
        fill(context, panelLeft, panelTop, panelLeft + 1, panelBottom, PANEL_BORDER_COLOR);
        fill(context, panelRight - 1, panelTop, panelRight, panelBottom, PANEL_BORDER_COLOR);

        drawCenteredString(context, font, title, width / 2, panelTop + 16, TEXT_COLOR);
        ...
        drawCenteredString(context, font, new TranslationTextComponent(firstLineKey), width / 2, textY, MUTED_TEXT_COLOR);
        drawCenteredString(context, font, new TranslationTextComponent(secondLineKey), width / 2, textY + 18, MUTED_TEXT_COLOR);
        if (state == State.RESTART_FAILED) {
            ...
            drawWrappedCenteredText(context, new TranslationTextComponent("screen.universal_config.restart_failed_line3"),
                    textY + 36, MUTED_TEXT_COLOR);
        } else if (cancelError) {
            drawWrappedCenteredText(context, new TranslationTextComponent("screen.universal_config.cancel_failed"),
                    textY + 36, 0xFFFF7777);
        }
    ...
    private void drawWrappedCenteredText(MatrixStack context, ITextComponent message, int startY, int color) {
        int maxTextWidth = Math.max(1, panelWidth() - PANEL_MARGIN * 2);
        List<IReorderingProcessor> lines = font.split(message, maxTextWidth);
        int lineY = startY;
        for (IReorderingProcessor line : lines) {
            int lineX = width / 2 - font.width(line) / 2;
            font.draw(context, line, lineX, lineY, color);
            lineY += 10;
        }
    }
    ...
    private int measureButton(ITextComponent label) {
        int measured = font.width(label) + 20;
        return Math.max(104, Math.min(150, measured));
    }

    private int pairButtonWidth(ITextComponent first, ITextComponent second) {
        return Math.max(measureButton(first), measureButton(second));
    }

    private int scheduledButtonWidth() {
        int action = pairButtonWidth(
                new TranslationTextComponent("screen.universal_config.apply_scheduled_later"),
                new TranslationTextComponent("screen.universal_config.restart_now"));
        int cancel = measureButton(new TranslationTextComponent("screen.universal_config.apply_scheduled_cancel"));
        return Math.max(action, cancel);
    }

    private int errorButtonWidth() {
        return pairButtonWidth(
                new TranslationTextComponent("screen.universal_config.back"),
                new TranslationTextComponent("screen.universal_config.quit"));
    }
    ...
    private static String titleKey(State state) {
        if (state == State.SCHEDULED) return "screen.universal_config.apply_scheduled_title";
        if (state == State.RESTART_FAILED) return "screen.universal_config.restart_failed_title";
        return "screen.universal_config.pending_invalid_title";
    }
}
