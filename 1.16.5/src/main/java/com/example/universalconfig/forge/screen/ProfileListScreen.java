...
public final class ProfileListScreen extends Screen {
    ...
    public ProfileListScreen(Screen parent) {
        super(new TranslationTextComponent("screen.universal_config.title"));
        this.parent = parent;
    }
    ...
    private void reload() {
        ...
        try {
            ...
            status = ex instanceof UniversalConfigException
                    ...
                    : new TranslationTextComponent("screen.universal_config.load_failed");
        }
    }
    ...
    private int pendingButtonWidth(ITextComponent label) {
    ...
    private void rebuildButtons() {
        ...
        for (int index = firstRow; index < lastRow; index++) {
            ...
            IFormattableTextComponent narration = new StringTextComponent(profileName(profile.manifest()));
        ...
        if (selected != null) {
            ...
            addButton(Button.builder(new TranslationTextComponent("screen.universal_config.use_profile"), button -> openConfirm(path))
                    .bounds(detailX(), primaryButtonY(), actionWidth, PRIMARY_BUTTON_HEIGHT).build());
            addButton(Button.builder(new TranslationTextComponent("screen.universal_config.more"), button -> {
        ...
        if (pendingImport != null) {
            ITextComponent restartLabel = new TranslationTextComponent("screen.universal_config.restart_now");
            ITextComponent cancelLabel = new TranslationTextComponent("screen.universal_config.apply_scheduled_cancel");
            ...
            addButton(Button.builder(cancelLabel, button -> cancelPendingApply())
                    .bounds(right - cancelWidth, pendingTop() + 5, cancelWidth, BUTTON_HEIGHT).build());
            addButton(Button.builder(restartLabel, button -> restartForPendingApply())
                    .bounds(right - cancelWidth - 6 - restartWidth, pendingTop() + 5, restartWidth, BUTTON_HEIGHT).build());
        }

        IFormattableTextComponent closeNarration = new TranslationTextComponent("screen.universal_config.close");
        addButton(Button.builder(new StringTextComponent("×"), button -> onClose())
    ...
    private void addMoreMenuButtons(Path path) {
        ...
        ITextComponent defaultLabel = new TranslationTextComponent(isDefaultProfile(path)
        ...
        addMoreMenuButton(Button.builder(new TranslationTextComponent("screen.universal_config.open_folder"), button -> openProfileDirectory())
                .bounds(x, y + BUTTON_HEIGHT + MENU_ITEM_GAP, width, BUTTON_HEIGHT).build());
        addMoreMenuButton(Button.builder(new TranslationTextComponent("screen.universal_config.rename"), button -> rename(path))
                .bounds(x, y + (BUTTON_HEIGHT + MENU_ITEM_GAP) * 2, width, BUTTON_HEIGHT).build());
        addMoreMenuButton(Button.builder(new TranslationTextComponent("screen.universal_config.duplicate"), button -> duplicate(path))
                .bounds(x, y + (BUTTON_HEIGHT + MENU_ITEM_GAP) * 3, width, BUTTON_HEIGHT).build());
        addMoreMenuButton(Button.builder(new TranslationTextComponent("screen.universal_config.backups"), button -> openBackups())
                .bounds(x, y + (BUTTON_HEIGHT + MENU_ITEM_GAP) * 4, width, BUTTON_HEIGHT).build());
        addMoreMenuButton(Button.builder(new TranslationTextComponent("screen.universal_config.delete"), button -> confirmDelete(path))
    ...
    private void addReorderButtons(int profileIndex, int cardY) {
        ...
        int y = cardY + 3;
        Button upButton = Button.builder(new StringTextComponent("\u2191"), button -> moveProfile(profileIndex, profileIndex - 1))
                .createNarration(ignored -> new TranslationTextComponent("screen.universal_config.move_up"))
                .bounds(x, y, REORDER_BUTTON_WIDTH, REORDER_BUTTON_HEIGHT).build();
        upButton.active = profileIndex > 0;
        Button downButton = Button.builder(new StringTextComponent("\u2193"), button -> moveProfile(profileIndex, profileIndex + 1))
                .createNarration(ignored -> new TranslationTextComponent("screen.universal_config.move_down"))
    ...
    private void openConfirm(Path path) {
        try {
            ...
            status = new TranslationTextComponent("screen.universal_config.open_confirm_failed");
        }
    }
    ...
    private void confirmImport(Path source) {
        try {
            ...
            }, new TranslationTextComponent("screen.universal_config.import_confirm", name),
                    new TranslationTextComponent("screen.universal_config.import_warning")));
        } catch (UniversalConfigException | RuntimeException ex) {
            FileOperationLogger.failure("OPEN_IMPORT_CONFIRM", source, "invalid dropped profile", ex);
            status = new TranslationTextComponent("screen.universal_config.import_failed");
            rebuildButtons();
        }
    }
    ...
    private void confirmDelete(Path path) {
        ...
        try {
            ...
            }, new TranslationTextComponent("screen.universal_config.delete_confirm", name),
                    new TranslationTextComponent("screen.universal_config.delete_warning")));
        } catch (RuntimeException ex) {
            FileOperationLogger.failure("OPEN_DELETE_CONFIRM", path, "failed", ex);
            status = new TranslationTextComponent("screen.universal_config.delete_failed");
        }
    }
    ...
    public void onFilesDrop(List<Path> paths) {
        if (paths == null || paths.size() != 1 || !isDroppedProfileFile(paths.get(0))) {
            status = new TranslationTextComponent("screen.universal_config.drop_one_profile");
            rebuildButtons();
            return;
        }
    ...
    private void cancelPendingApply() {
        ...
        try {
            ...
            status = new TranslationTextComponent("screen.universal_config.cancel_failed");
        } catch (RuntimeException ex) {
    ...
    private void handleActionFailure(Exception ex, String fallbackKey) {
        if (ex instanceof UniversalConfigException) {
            ...
            status = new TranslationTextComponent(fallbackKey);
        }
    }
    ...
    private void drawHeader(MatrixStack context) {
        ...
        drawCenteredString(context, font, new TranslationTextComponent("screen.universal_config.subtitle"),
        ...
        if (pendingImport != null) {
            ...
            int actionWidth = pendingButtonWidth(new TranslationTextComponent("screen.universal_config.restart_now"))
                    + pendingButtonWidth(new TranslationTextComponent("screen.universal_config.apply_scheduled_cancel")) + 6;
            int availableWidth = contentWidth() - actionWidth - 34;
            drawTrimmed(context, new TranslationTextComponent("screen.universal_config.pending_named", pendingProfileName()).getString(),
    ...
    private void drawListContent(MatrixStack context) {
        drawPanelHeader(context, contentLeft(), leftPanelRight(), new TranslationTextComponent("screen.universal_config.saved_settings"));
        if (profiles.isEmpty()) {
    ...
    private void drawDetailContent(MatrixStack context) {
        ProfileSummary selected = selectedProfile();
        ITextComponent heading = selected == null
                ? new TranslationTextComponent("screen.universal_config.profile_detail")
                : new StringTextComponent(profileName(selected.manifest()));
        ...
        y += 31;
        for (IReorderingProcessor line : descriptionLines(manifest)) {
        ...
        drawString(context, font, new TranslationTextComponent("screen.universal_config.included_settings"),
    ...
    private List<IReorderingProcessor> descriptionLines(ProfileManifest manifest) {
        return font.split(new StringTextComponent(descriptionSummary(manifest)), detailContentWidth());
    }
    ...
    private String environmentSummary(ProfileManifest manifest) {
        ...
        return new TranslationTextComponent("screen.universal_config.confirm_environment", minecraftVersion, loader).getString();
    }
    ...
    private String loaderSummary(ProfileManifest manifest) {
        ...
        return new TranslationTextComponent("screen.universal_config.loader_summary", loader, loaderVersion).getString();
    }
    ...
    private int drawInclude(MatrixStack context, boolean included, String key, int x, int y) {
        if (included) {
            ...
            drawTrimmed(context, new TranslationTextComponent(key).getString(), x + 14, y, detailWidth() - 34, 0xFFFFFFFF);
            return y + 15;
        }
        return y;
    }

    private void drawPanelHeader(MatrixStack context, int left, int right, ITextComponent heading) {
    ...
    private void drawProfileIcon(MatrixStack context, int x, int y, int size, String iconId) {
        ...
        // 1.16's ItemRenderer does not consume this screen's MatrixStack. Transforming only the context leaves the
    ...
    private String[] environmentLines(ProfileManifest manifest) {
        ...
        return new String[]{new TranslationTextComponent("screen.universal_config.minecraft_version",
    ...
    private static final class ProfileCardButton extends Button {
        private ProfileCardButton(int x, int y, int width, int height, IFormattableTextComponent narration, IPressable onPress) {
