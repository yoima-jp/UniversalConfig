...
public final class ProfileListScreen extends Screen {
    ...
    public void render(GuiGraphics context, int mouseX, int mouseY, float delta) {
        // 1.21.1の4引数版は標準のポストエフェクトまで有効にするため、独自UI全体がぼけない背景描画を使う。
    ...
    private void drawHeader(GuiGraphics context) {
        ...
        context.drawCenteredString(font, Component.translatable("screen.universal_config.subtitle"),
    ...
    private void drawPanels(GuiGraphics context) {
    ...
    private void drawListContent(GuiGraphics context) {
    ...
    private void drawDetailContent(GuiGraphics context) {
        ProfileSummary selected = selectedProfile();
        ...
        for (FormattedCharSequence line : descriptionLines(manifest)) {
            context.drawString(font, line, x, y, SECONDARY_TEXT_COLOR);
    ...
    private void drawPanelHeader(GuiGraphics context, int left, int right, Component heading) {
        context.fill(left + 2, mainTop() + 2, right - 2, mainTop() + PANEL_HEADER_HEIGHT, PANEL_HEADER_COLOR);
        context.drawString(font, heading, left + PANEL_PADDING, mainTop() + 8, 0xFFFFFFFF);
        context.fill(left + 2, mainTop() + PANEL_HEADER_HEIGHT - 1, right - 2, mainTop() + PANEL_HEADER_HEIGHT, DIVIDER_COLOR);
    }

    private void drawProfileCards(GuiGraphics context, int mouseX, int mouseY) {
        int firstRow = firstVisibleRow();
        int lastRow = lastVisibleRowExclusive();
    ...
    private void drawDetailScrollbar(GuiGraphics context) {
    ...
    private void drawMoreMenuOverlay(GuiGraphics context, int mouseX, int mouseY, float delta) {
    ...
    private void drawStatus(GuiGraphics context) {
    ...
    private void drawBorderedRect(GuiGraphics context, int left, int top, int right, int bottom,
